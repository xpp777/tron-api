...
func (c *Client) GetBlockByLimitNext(Transfer func(*model.TransferData)) {
	...
	body := c.getClient().PostVar(URL, g.Map{"startNum": c.startNum, "endNum": c.startNum + c.count, "visible": true})
	if body.IsEmpty() {
		return
	}
...
func (c *Client) transfer(ownerKey *ecdsa.PrivateKey, toAddress string, amount int64) (string, error) {
	url := fmt.Sprintf("%s/wallet/createtransaction", c.Url)
	ownerAddress := base58.EncodeCheck(crypto.PubkeyToAddress(ownerKey.PublicKey).Bytes())
	fmt.Println("ownerAddress:", ownerAddress, "\nToAddress:", toAddress, "\namount:", amount)
	body := c.getClient().PostVar(url, g.Map{
	...
	result := new(model.CreateTransaction)
	err :=  body.Struct(result)
	if err != nil {
		g.Log().Error(err)
		return "", err
	}
	RawData := new(core.TransactionRaw)
	base,err := json.Marshal(result.RawData)
	fmt.Println(string(base))
	err = jsonpb.UnmarshalString(string(base),RawData)
	if err != nil {
		g.Log().Error(err)
		return "", err
	}
	...
	//sign.ApiSignTransaction()
	fmt.Println(RawData)
	return "", nil
}

// 转账
func (c *Client) Sen(key *ecdsa.PrivateKey, contract, to string, amount decimal.Decimal) (string, error) {
	Type, Decimal := ChargeContract(contract)
	switch Type {
	case Trx:
		var amountdecimal = decimal.New(1, Decimal)
		amountac, _ := amount.Mul(amountdecimal).Float64()
		return c.transfer(key, to, int64(amountac))
	case "trc20":
		//var amountdecimal = decimal.New(1, Decimal)
		//amountac, _ := amount.Mul(amountdecimal).Float64()
